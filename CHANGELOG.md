...
### Added
- Archetype-specific skills: `aget-assess-client`, `aget-propose-engagement`
- Formal ontology: `ontology/ONTOLOGY_consultant.yaml` (6 concepts, 2 clusters)
...
- README structure: "Why Consultant?" value proposition
...
- **Documentation Files**: CAPABILITIES.md, COMPARISON_MATRIX.md, USAGE_GUIDE.md
...
- CI workflow updated for v3.0 schema compatibility
...
### Added
- **Layer 4 (.memory/) for Consultant Advisors**:
  - `.memory/` directory for advisor relationship state (Layer 4)
  - `.memory/clients/` for client context and relationship tracking
  - `.memory/engagements/` for engagement scope and progress tracking
...
- **AGENTS.md Updates**:
  ...
  - Documented .memory/ boundary rules and usage guidelines
  - Added `sessions/` to directory structure (v2.9 standard location)
  - Privacy considerations made conditional (not all consultants handle PII)
